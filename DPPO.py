...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            self.policy, self.value = self.AC_net(self.imageIn, 'ac', trainable=True)
            self.oldpolicy, self.oldvalue = self.AC_net(self.imageIn, 'oldac', trainable=False)
            ...
            if scope != 'global':
                ...
                self.lclip = tf.minimum(clip, tf.clip_by_value(ratio, 0.8, 1.2) * self.advantages)

                self.value_loss = 0.5 * tf.reduce_sum(tf.square(self.target_v - tf.reshape(self.value,[-1])))
                self.entropy = - tf.reduce_sum(self.policy * tf.log(self.policy))
                #self.policy_loss = -tf.reduce_sum(tf.log(self.responsible_outputs) * self.lclip)
                self.policy_loss = - tf.reduce_sum(self.lclip)
                ...
                self.soft_replace = [[tf.assign(ta, (1 - TAU) * ta + TAU * ea)]
                                     for ta, ea in zip(local_varsold, local_vars)]

    def AC_net(self, s, scope, trainable):
        with tf.variable_scope(scope):
...
class Worker():
    ...
    def train(self,rollout,sess,gamma,lam,bootstrap_value):
        ...
        """
        v_l,p_l,g_n,_ = sess.run([self.local_AC.value_loss,
                                          self.local_AC.policy_loss,
                                          self.local_AC.grad_norms,
                                          self.local_AC.apply_grads], feed_dict=feed_dict)
        """
        v_l,p_l = sess.run([self.local_AC.value_loss, self.local_AC.policy_loss], feed_dict=feed_dict)
        [sess.run(self.local_AC.apply_grads, feed_dict=feed_dict) for _ in range(update_steps)]
    ...
    def work(self,gamma,lam,sess,coord,saver):
        ...
        with sess.as_default(), sess.graph.as_default():                 
            while not coord.should_stop():
                ...
                #print ('vloss',v_l, 'ploss',p_l, 'qloss')
                ...
                if episode_count % 5 == 0 and episode_count != 0:
                    ...
                    if episode_count % 100 == 0 and self.name == 'worker_0':
                        saver.save(sess,self.model_path+'/ppo-'+str(episode_count)+'.cptk')
                        print ("Saved Model")
